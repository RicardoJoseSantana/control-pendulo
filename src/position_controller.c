...
static float g_kpx = 0.0; // Ganancia Proporcional para la posición (empezar bajo)
static float g_kix = 0.0; // Ganancia Integral para la posición (muy bajo)
static float g_kdx = 0.0; // Ganancia Derivativa para la posición
...
float pos_pid_get_kp(void) { return g_kpx; }
float pos_pid_get_ki(void) { return g_kix; }
float pos_pid_get_kd(void) { return g_kdx; }

// Funciones de sintonización
void pos_pid_set_kp(float kp) { g_kpx = kp; ESP_LOGI(TAG, "Position KpX set to: %f", g_kpx); }
void pos_pid_set_ki(float ki) { g_kix = ki; ESP_LOGI(TAG, "Position KiX set to: %f", g_kix); }
void pos_pid_set_kd(float kd) { g_kdx = kd; ESP_LOGI(TAG, "Position KdX set to: %f", g_kdx); }
